...
// The for (const element of all(conversationItemSelector)) loop was added to replace the non-compliant 
// array's .forEach() method, satisfying the project's strict linter rules for iterating over the results of all().
function init(): void {
...
// It was changed by adding the void operator before features.add to explicitly signal to the TypeScript linter that 
// the returned Promise is intentionally ignored, thereby satisfying the @typescript-eslint/no-floating-promises rule
// and resolving a critical CI failure.
void features.add(
